...
-   [Was ist eine vollständige Systemwiederherstellung?](Restore-a-full-system-from-an-existing-client-computer-backup.md#BKMK_WhatIs)
...
-   [Erstellen eines startbaren USB-Speichersticks zur Wiederherstellung eines Clientcomputers](Restore-a-full-system-from-an-existing-client-computer-backup.md#BKMK_CreateBootable)

-   [Verwenden des Assistenten für vollständige Wiederherstellung](Restore-a-full-system-from-an-existing-client-computer-backup.md#BKMK_Using)
...
9. Geben Sie auf der Seite **Anmelden an <yourservername \> ** im Textfeld **Benutzername den Namen** *<IhrServername \> * und im Textfeld **Kennwort** das Kennwort für das Administrator Konto ein, und klicken Sie dann auf **weiter**.
...
15. Auf der Seite **Wiederherzustellende Volumes auswählen** können Sie die wiederherzustellenden Volumes auswählen.
    ...
    2. Wenn das Quellvolume und das Zielvolume dieselbe Größe aufweisen oder das Quellvolume kleiner als das Zielvolume ist, wird zwischen beiden ein grüner Pfeil angezeigt. Wenn die Volumegrößen nicht übereinstimmen (das Quellvolume größer als das Zielvolume ist), wird zwischen dem Quellvolume und dem Zielvolume ein rotes X angezeigt.
       ...
       1. Klicken Sie auf **Datenträgerverwaltung ausführen (erweitert)**, und erstellen Sie ein neues Volume, das dieselbe Größe wie das vom System reservierte Volume aufweist.
          ...
          >    2. **DiskPart> auswählen** des Datenträgers # * \><* Datenträgers
          >    3. **DISKPART> clean**
          >    4. **DISKPART> convert gpt**
          >    5. **DISKPART>create partition efi size=** *100* (wobei *100* ein Beispiel für die Partitionsgröße in MB ist, die der Größe der ursprünglichen Partition entsprechen sollte)
          >    6. **DISKPART>create partition msr size=** *128* (wobei *128* ein Beispiel für die Partitionsgröße in MB ist, die der Größe der ursprünglichen Partition entsprechen sollte)
          >    7. **DISKPART> exit**
...
17. Auf der Seite **Wiederherstellen von <Computername \> von <Datum \> und Uhrzeit der Sicherung** wird der Fortschritt des Wiederherstellungs Vorgangs angegeben.
