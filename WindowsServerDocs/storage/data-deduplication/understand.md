...
1. **Bei der Optimierung sollten keine Schreibvorgänge auf den** Datenträger durchlaufen werden. Die Datendeduplizierung optimiert Daten mithilfe eines Nachbearbeitungs Modells. Alle Daten werden in nicht optimierter Form auf den Datenträger geschrieben und später durch Datendeduplizierung optimiert.

2. Bei der **Optimierung sollte die Zugriffs Semantik nicht geändert** werden. Benutzer und Anwendungen, die auf Daten auf einem optimierten Volume zugreifen, wissen nicht, dass die Dateien, auf die Sie zugreifen, dedupliziert wurden.
...
| <a id="dedup-term-optimization"></a>Optimization (Optimierung) | Eine Datei gilt als durch die Datendeduplizierung optimiert (bzw. dedupliziert), wenn sie in Blöcke aufgeteilt wurde und ihre eindeutigen Blöcke im Blockspeicher gespeichert wurden. |
