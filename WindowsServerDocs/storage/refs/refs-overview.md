...
- **[Durch Spiegelung beschleunigte Parität](./mirror-accelerated-parity.md)** -Durch Spiegelung beschleunigte Parität bietet hohe Leistung und eine effizient Speicherkapazität für Ihre Daten. 
    ...
    - Nachdem diese Ebenen konfiguriert wurden, verwendet ReFS diese, um schnell Speicherplatz für „Hot Data” anzubieten und effizient Speicherkapazität für „Cold Data” bereitzustellen:
        ...
        - Bei Verwendung eines [Hybridbereitstellungsmodells](../storage-spaces/choosing-drives.md) (einer Kombination aus Flash- und HDD-Laufwerk), hilft [ein Cache im direkten Speicherplatz](../storage-spaces/understand-the-cache.md) bei der Beschleunigung der Lesevorgänge und bei der Reduzierung der Auswirkung der Datenfragmentationseffekte von virtualisierten Workloads. Bei einer reinen Flash-Bereitstellung werden die Lesevorgänge ebenfalls auf der Leistungsebene ausgeführt. 

>[!NOTE]
>Für Serverbereitstellungen wird die durch Spiegelung beschleunigte Parität nur auf [direkten Speicherplätzen](../storage-spaces/storage-spaces-direct-overview.md) unterstützt.
...
- **Variable Clustergrößen** -ReFS unterstützt Clustergrößen von sowohl 4-KB als auch 64KB. 4K ist die empfohlene Clustergröße für die meisten Bereitstellungen, doch 64KB Cluster sind für große, aufeinander folgende E/A-Workloads am geeignetsten.
...
- Integrity Streams, Onlinereparaturvorgänge und alternative Datenkopien ermöglichen ReFS und direkten Speicherplätzen gemeinsam Schäden in Metadaten und Daten zu erkennen und diese zu beheben. 
- ReFS bietet die Funktionalität zur Skalierung und unterstützt große Datasets. 

### <a name="storage-spaces-with-sas-drive-enclosures"></a>Speicherplätze mit SAS-Laufwerksgehäuse ###
Das Bereitstellen von ReFS für Speicherplätze mit freigegebenem SAS-Gehäuse eignet sich für das Hosten von Archivierungsdaten und das Speichern von Benutzerdokumenten:
- Integrity Streams, Onlinereparaturvorgänge und alternative Datenkopien ermöglichen ReFS und Speicherplätzen gemeinsam Schäden in Metadaten und Daten zu erkennen und diese zu beheben. 
...
Das Bereitstellen von ReFS auf Basisfestplatten eignet sich am besten für Anwendungen, die ihre eigenen Software-Resilienz- und Verfügbarkeitslösungen aufweisen. 
...
>[!NOTE]
>ReFS ist nicht auf SAN angeschlossenen Speichergeräten verfügbar.
...
| Maximale Dateigröße | 18 EB (Exabyte)  | 18 EB (Exabyte)                |
| Maximale Volumegröße | 4.7 ZB (Zettabytes)                           | 256 TB                |
...
| Funktionen       | ReFS                                        | NTFS |
|---------------------------|------------------|-----------------------|
| BitLocker-Verschlüsselung | Ja. | Ja. |
| Volumes mit freigegebener Unterstützung (Cluster Shared Volume, CSV) | Ja | Ja |
| Weiche Links | Ja | Ja |
