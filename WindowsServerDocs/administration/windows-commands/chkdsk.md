---
title: chkdsk
description: Referenz Thema für den chkdsk-Befehl, mit dem das Dateisystem und die Dateisystem Metadaten eines Volumes auf logische und physische Fehler überprüft werden.
...
| /forceofflinefix | Nur mit NTFS verwenden (muss mit **/Scan**verwendet werden). Alle Online Reparaturen umgehen alle gefundenen Fehler werden in die Warteschlange für die Offline Reparatur eingereiht (z. b. `chkdsk /spotfix`). |
...
- Wenn Sie auf `chkdsk /f*` einem sehr großen Datenträger oder einem Datenträger mit einer sehr großen Anzahl von Dateien (z. b. Millionen von Dateien `chkdsk /f` ) verwenden, kann es sehr lange dauern, bis der Vorgang beendet ist.
