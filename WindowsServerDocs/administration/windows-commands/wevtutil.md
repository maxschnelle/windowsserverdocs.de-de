...
|{SL \| Set-Log} \<LogName> [/e:\<aktivierte>] [/i\<: Isolation>] [/LFN\<: logPath>] [/RT\<: Retention>] [/ab\<: Auto>] [/MS\<: MaxSize>] [/l\<: Level>] [/k\<: Keywords>] [/ca\<: Channel>] [/c\<: config>]|Ändert die Konfiguration des angegebenen Protokolls.|
...
|{im \| Install-Manifest} \<Manifest>|Installiert Ereignis Herausgeber und-Protokolle aus einem Manifest. Weitere Informationen zu Ereignis Manifesten und zur Verwendung dieses Parameters finden Sie im Windows-Ereignisprotokoll-SDK auf der MSDN-Website (Microsoft[https://msdn.microsoft.com](https://msdn.microsoft.com)Developer Network) ().|
|{um \| Uninstall-Manifest} \<Manifest>|Deinstalliert alle Verleger und Protokolle aus einem Manifest. Weitere Informationen zu Ereignis Manifesten und zur Verwendung dieses Parameters finden Sie im Windows-Ereignisprotokoll-SDK auf der MSDN-Website (Microsoft[https://msdn.microsoft.com](https://msdn.microsoft.com)Developer Network) ().|
|{QE \| -Abfrage-Ereignisse} \<Pfad> [/LF:\<Logfile>] [/sq:\<structquery>] [/q:\<Query>] [/BM:\<Bookmark>] [/SBM:\<savebm>] [/RD:\<Direction>] [/f:\<Format>] [/l:\<locale>] [/c:\<count>] [/e:\<Element>]|Liest Ereignisse aus einem Ereignisprotokoll, aus einer Protokolldatei oder mithilfe einer strukturierten Abfrage. Standardmäßig geben Sie einen Protokollnamen für Pfad \<> an. Wenn Sie jedoch die Option **/LF** verwenden, muss \<Path> ein Pfad zu einer Protokolldatei sein. Wenn Sie den **/SQ** -Parameter verwenden \<, muss path> ein Pfad zu einer Datei sein, die eine strukturierte Abfrage enthält.|
...
|    /l:\<Ebene>     |                                                                                                                                                                     Definiert den Filter für die Ebene des Protokolls. \<Die Ebene> kann ein beliebiger gültiger Wert für die Ebene sein. Diese Option gilt nur für Protokolle mit einer dedizierten Sitzung. Sie können einen Ebenen-Filter entfernen, <Level> indem Sie auf 0 festlegen.                                                                                                                                                                      |
|   /k:\<Schlüsselwörter>   |                                                                                                                                                                                         Gibt den Schlüsselwort Filter für das Protokoll an. \<Schlüsselwörter> können eine beliebige gültige Schlüsselwort Maske für 64 Bit sein Diese Option gilt nur für Protokolle mit einer dedizierten Sitzung.                                                                                                                                                                                         |
|   /ca:\<Kanal>   |                                                                                                                   Legt die Zugriffsberechtigung für ein Ereignisprotokoll fest. \<Channel> ist eine Sicherheits Beschreibung, die die Security Deskriptor Definition Language (SDDL) verwendet. Weitere Informationen zum SDDL-Format finden Sie auf der MSDN-Website (Microsoft Developer Network)[https://msdn.microsoft.com](https://msdn.microsoft.com)().                                                                                                                    |
|    /c:\<Konfigurations>    |                                                                                                                                  Gibt den Pfad zu einer Konfigurationsdatei an. Diese Option bewirkt, dass die Protokoll Eigenschaften aus der Konfigurationsdatei gelesen werden, \<die in der Konfigurations> definiert ist. Wenn Sie diese Option verwenden, dürfen Sie keinen <Logname> Parameter angeben. Der Protokoll Name wird aus der Konfigurationsdatei gelesen.                                                                                                                                   |
...
|   /LF:\<Logfile->   |                                                                                                                                                                                  Gibt an, dass die Ereignisse aus einem Protokoll oder aus einer Protokolldatei gelesen werden sollen. \<Die Protokolldatei> kann "true" oder "false" sein. True gibt an, dass der-Parameter für den-Befehl der Pfad zu einer Protokolldatei ist.                                                                                                                                                                                   |
...
|   /e:\<Element>    |                                                                                                                                                           Schließt ein root-Element ein, wenn Ereignisse in XML angezeigt werden. \<Element> ist die Zeichenfolge, die Sie innerhalb des Stamm Elements benötigen. Beispielsweise würde **/e: root** zu XML führen, das das Stamm Element Paar \<root></root>enthält.                                                                                                                                                            |
|  /oW:\<> überschreiben  |                                                                                                                                                                 Gibt an, dass die Exportdatei überschrieben werden soll. \<Überschreiben> kann true oder false sein. Wenn true und die in <Exportfile> angegebene Exportdatei bereits vorhanden ist, wird Sie ohne Bestätigung überschrieben.                                                                                                                                                                 |
|   /BU:\<Sicherungs>    |                                                                                                                                                                                                      Gibt den Pfad zu einer Datei an, in der die gelöschten Ereignisse gespeichert werden. Fügen Sie die Erweiterung. evtx in den Namen der Sicherungsdatei ein.                                                                                                                                                                                                       |
...
|   /u:\<username->   |                                                                                                                                                                          Gibt einen anderen Benutzer an, der sich an einem Remote Computer anmelden soll. \<Der Benutzername> ist ein Benutzername im Format "Domäne \ Benutzer" oder "Benutzer". Diese Option ist nur anwendbar, wenn die **/r** -Option angegeben wird.                                                                                                                                                                          |
|   /p:\<Kennwort>   |                                                                                                                                               Gibt das Kennwort für den Benutzer an. Wenn die **/u** -Option verwendet wird und diese Option nicht angegeben ist \<oder wenn Password> ist *, wird der Benutzer zur Eingabe eines Kennworts aufgefordert. Diese Option ist nur anwendbar, wenn \* \*die/u* \* -Option angegeben wird.                                                                                                                                                |
|     /a:\<auth->     |                                                                                                                                                                                             Definiert den Authentifizierungstyp zum Herstellen einer Verbindung mit einem Remote Computer. \<Die Authentifizierung> kann Default, Aushandlungs-, Kerberos-oder NTLM-Werte sein. Der Standardwert ist "aushandeln".                                                                                                                                                                                              |
|  /Uni:\<Unicode->   |                                                                                                                                                                                                             Zeigt die Ausgabe in Unicode an. \<Unicode-> können true oder false sein. Wenn <Unicode> den Wert true hat, erfolgt die Ausgabe in Unicode.                                                                                                                                                                                                             |
