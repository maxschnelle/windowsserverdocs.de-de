---
title: bei
description: Windows-Befehls Thema für **at**, das Befehle und Programme plant, die auf einem Computer zu einem bestimmten Zeitpunkt (Datum und Uhrzeit) ausgeführt werden.
...
| `every:` | Führt den *Befehl* an allen angegebenen Tages-oder Wochentagen (z. b. jeden Donnerstag oder am dritten Tag jedes Monats) aus. |
...
| `next:` | Führt den *Befehl* beim nächsten Vorkommen des Tages aus (z. b. nächster Donnerstag). |
...
- **Schtasks** ist ein weiteres Befehlszeilen-Planungstool, das Sie zum Erstellen und Verwalten geplanter Aufgaben verwenden können. Weitere Informationen zu " **Schtasks**" finden Sie unter " [Schtasks](schtasks.md)".

- Um **mit**zu verwenden, müssen Sie Mitglied der lokalen Administratoren Gruppe sein.

- **bei** wird "cmd. exe", der Befehls Interpreter, vor dem Ausführen von Befehlen nicht automatisch geladen. Wenn Sie keine ausführbare Datei (exe-Datei) ausführen, müssen Sie "cmd. exe" wie folgt explizit am Anfang des Befehls laden:
...
- Mit geplante Befehle **bei** Ausführung als Hintergrundprozesse. Die Ausgabe wird nicht auf dem Computerbildschirm angezeigt. Um die Ausgabe in eine Datei umzuleiten, verwenden Sie das Umleitungs Symbol `(>)`. Wenn Sie die Ausgabe in eine Datei umleiten, müssen Sie das Escapesymbol `(^)` vor dem Umleitungs Symbol verwenden, unabhängig davon, ob Sie **an** der Befehlszeile oder in einer Batchdatei verwenden. Geben Sie z. b. Folgendes ein, um die Ausgabe an Output. Text umzuleiten:
