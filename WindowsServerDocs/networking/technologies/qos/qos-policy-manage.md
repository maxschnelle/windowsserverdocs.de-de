...
Die URL muss in Form von [RFC 1738](https://tools.ietf.org/html/rfc1738)entsprechen `http[s]://<hostname\>:<port\>/<url-path>` . Sie können einen Platzhalter, `‘*'` , für `<hostname>` und/oder verwenden, `<port>` z. b. `https://training.\*/, https://\*.\*` , aber der Platzhalter darf keine Teil Zeichenfolge von `<hostname>` oder bezeichnen `<port>` .

Anders ausgedrückt: weder `https://my\*site/` noch `https://\*training\*/` ist gültig.

Optional können Sie **Unterverzeichnisse und Dateien einschließen** aktivieren, um den Abgleich für alle Unterverzeichnisse und Dateien nach einer URL auszuführen. Wenn diese Option beispielsweise aktiviert ist und die URL lautet `https://training` , werden Anforderungen für eine gute Entsprechung von der QoS-Richtlinie berücksichtigt ` https://training/video` .
...
Das TCP-Empfangs Fenster wurde in Windows Server 2012, Windows 8, Windows Server 2008 R2, Windows Server 2008 und Windows Vista aus früheren Windows-Versionen geändert. In früheren Versionen von Windows wurde das Fenster für die TCP-Empfangsseite auf maximal 64 Kilobyte (KB) beschränkt, während Windows Server 2012, Windows 8, Windows Server 2008 R2, Windows Server 2008 und Windows Vista das Empfangs seitige Fenster dynamisch auf bis zu 16 Megabyte (MB) skalieren. In der eingehenden TCP-Datenverkehrs Steuerung können Sie den Durchsatz für den eingehenden Durchsatz steuern, indem Sie den maximalen Wert festlegen, auf den das TCP-Empfangs Fenster vergrößert werden kann. Die Ebenen entsprechen den folgenden maximalen Werten.
...
Die [Wi-Fi-Allianz](https://go.microsoft.com/fwlink/?LinkId=160769) hat eine Zertifizierung für Wireless Multimedia \( WMM eingerichtet \) , die vier Zugriffs Kategorien definiert \( WMM_AC, \) um den Netzwerk Datenverkehr zu priorisieren, der in einem WLAN-Drahtlos Netzwerk übertragen wird \- . Die Zugriffs Kategorien lauten \( in der Reihenfolge der höchsten Priorität \) : sprach-, Video-, Best-und Background-Werte, die als VO, VI, be und BK abgekürzt werden. Die WMM-Spezifikation definiert, welche DSCP-Werte mit den vier Zugriffs Kategorien übereinstimmen:
...
Sie können QoS-Richtlinien erstellen, die diese DSCP-Werte verwenden, um sicherzustellen, dass tragbare Computer mit WLAN- \- zertifizierten™ für WMM-drahtlos Adapter eine priorisierte Behandlung erhalten, wenn Sie mit WLAN- \- zertifiziert für WMM-Zugriffspunkte verknüpft sind.
...
Beispielsweise gibt policy_A nur einen Anwendungsnamen (app.exe) an, und policy_B gibt die Ziel-IP-Adresse 192.168.1.0/24 an. Wenn diese QoS-Richtlinien \( in Konfliktapp.exe Senden von Datenverkehr an eine IP-Adresse im Bereich von 192.168.4.0/24 \) , wird policy_A angewendet.
