...
- [Konfigurieren von Drahtlos Netzwerken \(IEEE 802,11\)-Richtlinien](#bkmk_policies)
...
- **SSID**. Geben Sie den Namen des drahtlos Netzwerks\(s\) an \(z. b. examplewlan\). Dies ist der Name, der drahtlosen Clients angekündigt wird.

- **Verschlüsselung**. Geben Sie WPA2\-Enterprise \(bevorzugt\) oder WPA\-Enterprise an, und entweder AES \(bevorzugtes\) oder TKIP-Verschlüsselungs Verschlüsselungsverfahren, je nachdem, welche Versionen von den Netzwerkadaptern des drahtlos Client Computers unterstützt werden.
...
- **DHCP-Dienst**. Wenn Ihr drahtlos Zugriffspunkt über eine integrierte\-im DHCP-Dienst verfügt, deaktivieren Sie diese.  
...
- Der **UDP-Port\(s\)** . Standardmäßig verwendet NPS die UDP-Ports 1812 und 1645 für Authentifizierungs Nachrichten und die UDP-Ports 1813 und 1646 für Buchhaltungs Nachrichten. Es wird empfohlen, dass Sie dieselben UDP-Ports auf Ihren APS verwenden. Wenn Sie jedoch einen gültigen Grund für die Verwendung anderer Ports haben, stellen Sie sicher, dass Sie die APS nicht nur mit den neuen Portnummern konfigurieren, sondern auch alle NPSS so konfigurieren, dass Sie dieselben Portnummern wie die APS verwenden. Wenn APS und NPSS nicht mit denselben UDP-Ports konfiguriert sind, kann NPS keine Verbindungsanforderungen von APS empfangen oder verarbeiten, und alle drahtlosen Verbindungsversuche im Netzwerk können nicht ausgeführt werden.

- **VSAs**. Einige drahtlos Zugriffspunkte erfordern Anbieter\-bestimmten Attributen \(VSAs-\), um vollständige WLAN-Funktionen bereitzustellen. VSAs werden in NPS-Netzwerk Richtlinien hinzugefügt.
...
Mit diesem Verfahren können Sie in der Microsoft-Verwaltungskonsole \(MMC\)\-Snap-in "Active Directory Benutzer und Computer" eine drahtlos Sicherheitsgruppe erstellen.  
...
1. Klicken Sie auf **Start**, zeigen Sie auf **Verwaltung**, und klicken Sie dann auf **Active Directory-Benutzer und -Computer**. Das Active Directory Benutzer-und Computer-Snap\-in wird geöffnet. Klicken Sie auf den Knoten für Ihre Domäne, wenn diese noch nicht ausgewählt ist. Wenn Ihre Domäne beispielsweise den Namen „beispiel.com“ besitzt, klicken Sie auf **beispiel.com**.

2. Klicken Sie im Detailfenster mit der rechten\-auf den Ordner, in dem Sie eine neue Gruppe hinzufügen möchten \(klicken Sie beispielsweise mit der rechten\-auf **Benutzer**\), zeigen Sie auf **neu**, und klicken Sie dann auf **Gruppe**.
...
Mithilfe dieses Verfahrens können Sie einen Benutzer, einen Computer oder eine Gruppe zur drahtlos Sicherheitsgruppe in der Active Directory-Benutzer und-Computer \(MMC\) Snap\-in hinzufügen.
...
2. Doppel\-klicken Sie im Detailbereich auf den Ordner, der die drahtlose Sicherheitsgruppe enthält.

3. Klicken Sie im Detailfenster mit der rechten\-auf die Gruppe drahtlose Sicherheit, und klicken Sie dann auf **Eigenschaften**. Das Dialogfeld **Eigenschaften** für die Sicherheitsgruppe wird geöffnet.
...
## <a name="configure-wireless-network-ieee-80211-policies"></a><a name="bkmk_policies"></a>Konfigurieren von Drahtlos Netzwerken \(IEEE 802,11\)-Richtlinien

Führen Sie die folgenden Schritte aus, um das Drahtlos Netzwerk \(IEEE 802,11\) Policies Gruppenrichtlinie Erweiterung zu konfigurieren:

- [Öffnen oder hinzufügen und Öffnen eines Gruppenrichtlinie Objekts](#bkmk_opengpme)

- [Aktivieren des standardmäßigen drahtlos Netzwerks \(IEEE 802,11\)-Richtlinien](#bkmk_activate)
...
2. Doppel\-klicken Sie im linken Bereich auf Ihre Gesamtstruktur. Doppel\-klicken Sie beispielsweise auf Gesamtstruktur **: example.com**.  
...
4. Führen Sie eine der folgenden Aktionen aus:
    ...
    -   **Wenn Sie ein vorhandenes Gruppenrichtlinien Objekt auf\-Ebene für die Bearbeitung öffnen möchten**, doppelklicken Sie auf die Domäne mit dem zu verwaltenden Gruppenrichtlinie Objekt, klicken Sie mit der rechten\-auf die Domänen Richtlinie, die Sie verwalten möchten, z. b. die Standard Domänen Richtlinie, und klicken Sie auf **Bearbeiten**. **Gruppenrichtlinienverwaltungs-Editor** wird geöffnet.

    -   Wenn Sie **ein neues Gruppenrichtlinie Objekt erstellen und zum Bearbeiten öffnen**möchten, klicken Sie mit der rechten\-auf die Domäne, für die Sie ein neues Gruppenrichtlinie Objekt erstellen möchten, und klicken Sie dann auf Gruppenrichtlinien Objekt **in dieser Domäne erstellen und verknüpfen**.
        ...
        Klicken Sie mit der rechten\-auf das neue Gruppenrichtlinie Objekt, und klicken Sie dann auf **Bearbeiten**. **Gruppenrichtlinienverwaltungs-Editor** wird geöffnet.

Im nächsten Abschnitt verwenden Sie Gruppenrichtlinienverwaltungs-Editor, um eine drahtlose Richtlinie zu erstellen.

### <a name="activate-default-wireless-network-ieee-80211-policies"></a><a name="bkmk_activate"></a>Aktivieren des standardmäßigen drahtlos Netzwerks \(IEEE 802,11\)-Richtlinien

In diesem Verfahren wird beschrieben, wie Sie das standardmäßige Drahtlos Netzwerk \(IEEE 802,11\)-Richtlinien mithilfe der Gruppenrichtlinienverwaltungs-Editor \(GP\)Me aktivieren.

>[!NOTE]
>Nachdem Sie die Version von **Windows Vista und höhere Versionen** des drahtlos Netzwerks \(IEEE 802,11\)-Richtlinien oder **Windows XP** -Version aktiviert haben, wird die Versions Option automatisch aus der Liste der Optionen entfernt, wenn Sie mit der rechten\-auf **Drahtlos Netzwerk \(IEEE 802,11\)-Richtlinien**klicken. Dies liegt daran, dass nach der Auswahl einer Richtlinien Version die Richtlinie im Detailbereich der GPME hinzugefügt wird, wenn Sie den Knoten **Drahtlos Netzwerk \(IEEE 802,11\) Richtlinien** auswählen. Dieser Status bleibt bestehen, es sei denn, Sie löschen die drahtlose Richtlinie. zu diesem Zeitpunkt kehrt die Version der drahtlos Richtlinie zur richtigen\-klicken Sie für das **Drahtlos Netzwerk \(IEEE 802,11\) Richtlinien** in der GPME auf das Menü. Außerdem werden die drahtlos Richtlinien nur im Detailbereich der GPME aufgeführt, wenn der Knoten **Drahtlos Netzwerk \(IEEE 802,11\) Policies** ausgewählt ist.

Sie müssen mindestens Mitglied der Gruppe **Domänen-Admins** oder einer entsprechenden Gruppe sein, um dieses Verfahren ausführen zu können.

#### <a name="to-activate-default-wireless-network-ieee-80211-policies"></a>So aktivieren Sie das standardmäßige Drahtlos Netzwerk \(IEEE 802,11\)-Richtlinien  
...
2. \-Doppelklicken Sie in der GPME im linken Bereich auf **Computer Konfiguration**, Doppel\-klicken Sie **auf Richtlinien**, Doppel\-klicken Sie auf **Windows-Einstellungen**, und Doppel\-klicken Sie dann auf **Sicherheitseinstellungen**.

![802.1 x-drahtlos Gruppenrichtlinie](../../../media/Wireless-GP/Wireless-GP.jpg)

3. Klicken Sie in den **Sicherheitseinstellungen**mit der rechten\-auf **Drahtlos Netzwerk \(IEEE 802,11\) Richtlinien**, und klicken Sie dann auf **neue drahtlos Richtlinie für Windows Vista und spätere Versionen erstellen**. 
...
Mithilfe der Verfahren in diesem Abschnitt können Sie das Drahtlos Netzwerk \(IEEE 802,11\)-Richtlinie konfigurieren. Diese Richtlinie ermöglicht Ihnen das Konfigurieren von Sicherheits-und Authentifizierungs Einstellungen, das Verwalten von drahtlos Profilen und das Angeben von Berechtigungen für Drahtlos Netzwerke, die nicht als bevorzugte Netzwerke konfiguriert sind.

- [Konfigurieren eines drahtlos Verbindungs Profils für die Peer-\-MS\-CHAP v2](#bkmk_configureprofile)  
...
#### <a name="configure-a-wireless-connection-profile-for-peap-ms-chap-v2"></a><a name="bkmk_configureprofile"></a>Konfigurieren eines drahtlos Verbindungs Profils für die Peer-\-MS\-CHAP v2
...
##### <a name="to-configure-a-wireless-connection-profile-for-peap-ms-chap-v2"></a>So konfigurieren Sie ein drahtlos Verbindungsprofil für die Peer-\-MS\-CHAP v2
...
8. Klicken Sie auf die Registerkarte **Sicherheit** und auf **Erweitert**, und konfigurieren Sie dann Folgendes:
    ...
    4. Wenn Ihr drahtlos Zugriffspunkt für die vor\-Authentifizierung konfiguriert ist, wählen Sie unter **schnelles Roaming**die Option **dieses Netzwerk verwendet vor\-Authentifizierung**aus.

9. Um anzugeben, dass die drahtlose Kommunikation die Standards der Standard-140\-2 erfüllt, wählen Sie **Kryptografie in\-2 Certified-Modus von "fps 140" aus**.

10. Klicken Sie auf **OK** , um zur Registerkarte **Sicherheit** zurückzukehren. Wählen Sie unter **Sicherheitsmethoden für dieses Netzwerk auswählen**bei **Authentifizierung**die Option **WPA2\-Enterprise** aus, wenn Sie von Ihrem drahtlosen Zugriffspunkt und den Netzwerkadaptern des drahtlosen Clients unterstützt wird. Wählen Sie andernfalls **WPA\-Enterprise**aus.
...
14. Wählen Sie unter **Vertrauenswürdige Stamm Zertifizierungs**stellen die vertrauenswürdige Stamm Zertifizierungsstelle \(ca-\) aus, die das Serverzertifikat für Ihren NPS ausgestellt hat.
...
15. Wählen Sie in der Liste **Authentifizierungsmethode auswählen** die Option **sicheres Kennwort \(EAP\-MS\-CHAP v2\)** aus.

16. Klicken Sie auf **Konfigurieren**. Überprüfen Sie im Dialogfeld **EAP-MSCHAPv2-Eigenschaften** , ob die Option **eigenen Windows-Anmelde Namen und Kennwort \(und Domäne automatisch verwenden, wenn eine\)** ausgewählt ist, und klicken Sie auf **OK**
...
19. Um anzugeben, dass die Benutzeridentität in Phase 1 der Authentifizierung maskiert wird, wählen Sie **Identitätsschutz aktivieren**aus, und geben Sie im Textfeld einen anonymen Identitäts Namen ein, oder lassen Sie das Textfeld leer.
    ...
    > - Der Datenschutz der EAP-Identität wird von bestimmten EAP-Methoden bereitgestellt, bei denen eine leere oder eine anonyme Identität \(von der tatsächlichen Identität abweicht\) die als Antwort auf die EAP-Identitäts Anforderung gesendet wird. Die Identität wird von der Peer-ID während der Authentifizierung zweimal gesendet. In der ersten Phase wird die Identität als nur-Text gesendet, und diese Identität wird für Routing Zwecke und nicht für die Client Authentifizierung verwendet. Die tatsächliche Identität –, die für die Authentifizierung verwendet wird – wird während der zweiten Phase der Authentifizierung innerhalb des sicheren Tunnels gesendet, der in der ersten Phase eingerichtet wurde. Wenn das Kontrollkästchen **Identitätsschutz aktivieren aktiviert** ist, wird der Benutzername durch den Eintrag ersetzt, der im Textfeld angegeben ist. Nehmen Sie beispielsweise an, dass **Identitätsschutz aktivieren** ausgewählt ist und der Identitätsdaten Schutz Alias **Anonym** im Textfeld angegeben wird. Bei einem Benutzer mit einem echten identitätsalias <strong>jdoe@example.com</strong>wird die in der ersten Phase der Authentifizierung gesendete Identität in <strong>anonymous@example.com</strong>geändert. Der Bereichs Teil der 1.-Phasen Identität wird nicht geändert, weil er für Routing Zwecke verwendet wird.  
...
Sie können Einstellungen auf der Registerkarte **Netzwerk Berechtigungen** für die Domänen Mitglieder konfigurieren, auf die die Richtlinien für Drahtlos Netzwerke \(IEEE 802,11\) angewendet werden.
...
- Hiermit wird Benutzern das Anzeigen von Netzwerktypen \(Ad-hoc-oder Infrastruktur\), denen der Zugriff verweigert wird, gestattet oder verweigert.
...
##### <a name="to-specify-additional-network-permissions-optional"></a>So geben Sie zusätzliche Netzwerk Berechtigungen \(optional an\)

1.  Konfigurieren Sie auf der Registerkarte **Netzwerk Berechtigungen** eine oder alle der folgenden Aktionen:  

    -   Wenn Sie Ihren Domänen Mitgliedern den Zugriff auf Ad-hoc-Netzwerke verweigern möchten, wählen Sie **Verbindungen mit AD\--hoc-Netzwerken**
    ...
    -   Um Ihren Domänen Mitgliedern das Anzeigen von Netzwerktypen \(Ad-hoc-oder Infrastruktur\) zu ermöglichen, denen der Zugriff verweigert wird, wählen Sie **Benutzern gestatten, abgelehnte Netzwerke anzuzeigen**.
...
Mit diesem Verfahren können Sie einen Server, auf dem der Netzwerk Richtlinien Server ausgeführt wird, \(NPS-\) in Active Directory Domain Services \(AD DS\) in der Domäne registrieren, in der das NPS Mitglied ist. Damit NPSS während der Autorisierung die Berechtigung zum Lesen der Wähl\-in Eigenschaften von Benutzerkonten erhält, muss jede NPS in AD DS registriert werden. Beim Registrieren eines NPS wird der Server der Sicherheitsgruppe **RAS-und IAS-Server** in AD DS hinzugefügt.
...
1. Klicken Sie auf dem NPS in **Server-Manager**auf **Extras, und klicken Sie dann**auf **Netzwerk Richtlinien Server**. Das NPS-Snap-in\-in wird geöffnet.

2. Klicken Sie mit der rechten\-auf **NPS \(lokalen\)** , und klicken Sie dann **auf Server in Active Directory registrieren**. Das Dialogfeld **Netzwerkrichtlinienserver** wird geöffnet.
...
Mithilfe dieses Verfahrens können Sie einen Zugriffs Steuerungspunkt konfigurieren, der auch als *Netzwerk Zugriffs Server \(NAS\)* bezeichnet wird, als Remote-Authentifizierungstyp\-in Benutzer Dienst \(RADIUS\) Client mithilfe des NPS-Snap\-in. 

>[!IMPORTANT]
>Clientcomputer, wie beispielsweise portable Funkcomputer oder andere Computer, auf denen Clientbetriebssysteme ausgeführt werden, sind keine RADIUS-Clients. RADIUS-Clients sind Netzwerk Zugriffs Server – z. b. drahtlos Zugriffspunkte, 802.1 x\-fähige Switches, virtuelles privates Netzwerk \(VPN-\) Servern und DFÜ-\-Server – da Sie das RADIUS-Protokoll für die Kommunikation mit RADIUS-Servern wie NPSS verwenden.
...
2. Doppel\-klicken Sie in der NPS-Snap-in\-auf **RADIUS-Clients und-Server**. Klicken Sie mit der rechten\-auf **RADIUS-Clients**und dann auf **neu**.
...
4. Geben Sie im **neuen RADIUS-Client**unter Anzeige **Name**einen anzeigen Amen für den drahtlos Zugriffspunkt ein.
    ...
    Wenn Sie z. b. einen drahtlosen Zugriffspunkt \(AP-\) mit dem Namen AP\-01 hinzufügen möchten, geben Sie **AP\-01**ein.

5. Geben Sie unter **Adresse \(IP-oder DNS-\)** die IP-Adresse oder den voll qualifizierten Domänen Namen \(FQDN-\) für NAS ein.
...
Mithilfe dieses Verfahrens können Sie die Verbindungs Anforderungs Richtlinien und Netzwerk Richtlinien erstellen, die für die Bereitstellung von "802.1 x\--fähigen drahtlos Zugriffs Punkten" als Remote-\-authentifizierungspunkte in Benutzer Dienst \(RADIUS\) Clients auf dem RADIUS-Server mit dem Netzwerk Richtlinien Server \(NPS-\)erforderlich sind.  
...
1. Öffnen Sie das NPS-Snap-in\-in. Wenn Sie nicht bereits ausgewählt ist, klicken Sie auf **NPS \(lokalen\)** . Wenn Sie das NPS-MMC-Snap\-in ausführen und Richtlinien für einen NPS-Remote Server erstellen möchten, wählen Sie den Server aus.
...
5.  Auf der Seite Assistent zum **Angeben von 802.1 x-Switches** in **RADIUS-Clients**werden alle 802.1 x-Switches und drahtlos Zugriffspunkte, die Sie als RADIUS-Clients im NPS-Snap-in hinzugefügt haben, in der\-angezeigt. Führen Sie eine der folgenden Aktionen durch:

    -   Zum Hinzufügen zusätzlicher Netzwerk Zugriffs Server \(nass\)wie z. b. drahtlos Zugriffs Punkten in **RADIUS-Clients**, klicken Sie auf **Hinzufügen**, und geben Sie dann unter **Neuer RADIUS-Client**die folgenden Informationen ein: Anzeige **Name**, **Adresse \(IP-Adresse, DNS-\)** und **gemeinsamer geheimer**Schlüssel.
    ...
    -   Wenn Sie ein NAS aus der Liste entfernen möchten, wählen Sie in **RADIUS-Clients**den NAS aus, und klicken Sie dann auf **Entfernen**.
        ...
        >[!WARNING]
        >Durch das Entfernen eines RADIUS-Clients aus dem Assistenten zum **Konfigurieren von 802.1 x** wird der Client aus der NPS-Konfiguration gelöscht. Alle Ergänzungen, Änderungen und Löschungen, die Sie im Assistenten zum **Konfigurieren von 802.1 x** für RADIUS-Clients vornehmen, werden in der NPS-Snap\-in im Knoten " **RADIUS-Clients** " unter **NPS** \/ **RADIUS-Clients und-Server**angezeigt. Wenn Sie z. b. den Assistenten zum Entfernen eines 802.1 x-Schalters verwenden, wird der Switch auch aus dem NPS-Snap-in\-entfernt.

6. Klicken Sie auf **Weiter**. Wählen Sie auf der Seite Assistent zum **Konfigurieren einer Authentifizierungsmethode** unter **Typ \(basierend auf Zugriffsmethode und Netzwerk\)Konfiguration** die Option **Microsoft: geschütztes EAP \(PEAP\)** aus, und klicken Sie dann auf **Konfigurieren**.

    >[!TIP]
    >Wenn Sie eine Fehlermeldung erhalten, die besagt, dass ein Zertifikat für die Verwendung mit der Authentifizierungsmethode nicht gefunden werden kann, und Sie haben Active Directory Zertifikat Dienste so konfiguriert, dass Zertifikate automatisch für RAS-und IAS-Server in Ihrem Netzwerk ausgestellt werden. Stellen Sie zunächst sicher, dass Sie die Schritte zum Registrieren von NPS in Active Directory Domain Services befolgt haben, und führen Sie dann die folgenden Schritte aus, um Gruppenrichtlinie zu aktualisieren: Klicken Sie auf **Start**, klicken Sie auf **Windows System**, klicken **Sie auf** **Ausführen**, **und geben Sie** , und drücken Sie dann die EINGABETASTE. Wenn der Befehl Ergebnisse zurückgibt, die anzeigen, dass sowohl Benutzer-als auch Computer Gruppenrichtlinie erfolgreich aktualisiert wurden, wählen Sie erneut **Microsoft: geschütztes EAP \(PEAP\)** aus, und klicken Sie dann auf **Konfigurieren**.
    >
    >Wenn Sie nach der Aktualisierung Gruppenrichtlinie weiterhin die Fehlermeldung erhalten, dass ein Zertifikat für die Verwendung mit der Authentifizierungsmethode nicht gefunden werden kann, wird das Zertifikat nicht angezeigt, weil es die Mindestanforderungen an das Serverzertifikat nicht erfüllt, wie im Hauptnetzwerk-Begleit Handbuch: bereitstellen [von Server Zertifikaten für Kabel-und drahtlos Bereitstellungen mit 802.1 x](https://technet.microsoft.com/windows-server-docs/networking/core-network-guide/cncg/server-certs/deploy-server-certificates-for-802.1x-wired-and-wireless-deployments)dokumentiert. Wenn dies der Fall ist, müssen Sie die NPS-Konfiguration einstellen, das Zertifikat, das für Ihre NPS-\(s\)ausgestellt wurde, widerrufen und dann die Anweisungen zum Konfigurieren eines neuen Zertifikats mithilfe des Bereitstellungs Handbuchs für Server Zertifikate befolgen.
...
7.  Vergewissern Sie sich, dass auf der Seite **geschützter EAP-Eigenschaften bearbeiten** unter **Zertifikat ausgestellt**das richtige NPS-Zertifikat ausgewählt ist, und gehen Sie dann wie folgt vor:
    ...
    >[!NOTE]
    >Überprüfen Sie, ob der Wert des **Ausstellers** für das im **Zertifikat ausgegebene**Zertifikat richtig ist. Beispielsweise ist der erwartete Aussteller für ein Zertifikat, das von einer Zertifizierungsstelle ausgegeben wird, die Active Directory Zertifikat Dienste \(AD CS\) mit dem Namen corp\dc1 in der Domäne contoso.com ausgeführt wird, **Corp\-DC1\-ca**.
    ...
    -   Um anzugeben, dass die Verbindung von drahtlosen Clients den Netzwerk Authentifizierungsprozess beenden soll, wenn der RADIUS-Server den kryptobindungstyp\-Länge\-Wert \(TLV-\)nicht enthält, wählen Sie **Clients ohne kryptoverbindung trennen**aus.  
...
9. Klicken Sie unter **Benutzergruppen angeben**auf **Hinzufügen**, und geben Sie dann den Namen der Sicherheitsgruppe ein, die Sie für die drahtlosen Clients im Active Directory Snap\-Benutzer und Computer in konfiguriert haben. Wenn Sie z. b. Ihre drahtlos Sicherheitsgruppe "drahtlos Gruppe" genannt haben, geben Sie **drahtlos Gruppe**ein. Klicken Sie auf **Weiter**.

10. Klicken Sie auf **Konfigurieren** , um RADIUS-Standard Attribute und Hersteller\-spezifische Attribute für virtuelles LAN \(VLAN\) nach Bedarf zu konfigurieren, und wie in der Dokumentation Ihres drahtlosen AP-Hardwareherstellers angegeben. Klicken Sie auf **Weiter**.
...
Domänen Mitglieds Benutzer mit Domänen\-verbundenen drahtlosen Client Computern können ein temporäres drahtlos Profil verwenden, um eine Verbindung mit einem 802.1\-x-authentifizierten Drahtlos Netzwerk herzustellen, ohne zuvor eine Verbindung mit dem kabelgebundenen LAN herzustellen. Dieses temporäre drahtlos Profil wird als *Bootstrap-drahtlos Profil*bezeichnet.

Ein Bootstrap-drahtlos Profil erfordert, dass der Benutzer die Anmelde Informationen für das Domänen Benutzerkonto manuell angibt, und überprüft das Zertifikat des Remote Authentifizierungs Wähl\-in User Service \(RADIUS\) Server, auf dem der Netzwerk Richtlinien Server \(n\)PS ausgeführt wird, nicht.
...
Außerdem überprüft der Client im Rahmen der Peer-\-MS\-CHAP v2-Authentifizierung mit dem neuen Profil anstelle des Bootstrap-Profils die Anmelde Informationen des RADIUS-Servers.

Nachdem Sie den Computer der Domäne hinzugefügt haben, verwenden Sie dieses Verfahren, um ein Bootstrap-drahtlos Profil für einmaliges Anmelden zu konfigurieren, bevor Sie den drahtlos Computer an die Domäne\-Mitglieds Benutzers verteilen.
...
1. Erstellen Sie ein Bootstrap-Profil mithilfe des Verfahrens in diesem Handbuch unter [Konfigurieren eines drahtlos Verbindungs Profils für die Peer-\-MS\-CHAP v2](#bkmk_configureprofile), und verwenden Sie die folgenden Einstellungen:
...
>[!NOTE]
>Wenn auf dem **anderen Benutzer** Bildschirm nicht der Text **Anmelden** bei und Ihr Domänen Name enthalten ist, geben Sie Ihren Benutzernamen im Format *Domäne\\Benutzer*ein. Wenn Sie sich beispielsweise bei der Domäne example.com mit einem Konto namens **User\-01**anmelden möchten, geben Sie **Beispiel\\Benutzer\-01**ein.
...
Mit dieser Methode führen Sie die Schritte im Abschnitt Allgemeine Schritte aus. Anschließend stellen Sie Ihren Domänen\-Mitglieds Benutzern die Anweisungen zur manuellen Konfiguration eines drahtlos Computers mit einem Bootstrap-drahtlos Profil bereit. Das Bootstrap-drahtlos Profil ermöglicht dem Benutzer das Herstellen einer drahtlos Verbindung und das anschließende beitreten zur Domäne. Nachdem der Computer der Domäne hinzugefügt und neu gestartet wurde, kann sich der Benutzer über eine drahtlose Verbindung bei der Domäne anmelden.
...
11. Klicken Sie auf **Einstellungen**. Die **Eigenschaften Seite geschützter EAP-\(\)** wird geöffnet.
...
13. Windows versucht dann, eine Verbindung mit dem Drahtlos Netzwerk herzustellen. In den Einstellungen des Bootstrap-drahtlos Profils wird angegeben, dass Sie Ihre Anmelde Informationen für die Domäne angeben müssen. Wenn Sie von Windows aufgefordert werden, einen Kontonamen und ein Kennwort einzugeben, geben Sie Ihre Domänen Konto-Anmelde Informationen wie folgt ein: *Domänen Name\\Benutzername*, *Domänen*
