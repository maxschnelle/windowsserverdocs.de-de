...
Der Netzwerk Controller überträgt Hyper-V-netzwerkvirtualisierungsrichtlinien an die virtuellen Netzwerke "Configuration Manager" und "Woodgrove" sowie RAS-Richtlinien zu den RAS-Gateways und Lasten Ausgleichs Richtlinien an die Multiplexers (muxes), die als Software Lastenausgleich-Pool konfiguriert sind.
...
Ebenso leitet das virtuelle Mandanten Netzwerk mit den Hyper-V-netzwerkvirtualisierungstechnologien Pakete direkt an die virtuellen RAS-Gateway-Computer weiter (ohne den Routen Reflektor kennen zu müssen) und dann an die Unternehmensstandorte über die Standort-zu-Standort-VPN-Tunnel.
