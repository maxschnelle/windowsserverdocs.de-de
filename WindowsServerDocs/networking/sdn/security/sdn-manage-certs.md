...
In diesem Thema erfahren Sie, wie Sie bei der Bereitstellung von Software-Defined Networking \(Sdn-\) in Windows Server 2016 Datacenter die Zertifikate für den Netzwerk Controller Northbound und Southbound-Kommunikation verwalten, und Sie verwenden System Center Virtual Machine Manager \(SCVMM-\) als Sdn-Verwaltungs Client.
...
Der Sdn in Windows Server 2016 Datacenter unterstützt sowohl selbst\-signierte als auch Zertifizierungsstelle \(ca\)signierten X. 509-Zertifikaten. Dieses Thema enthält Schritt-für-Schritt-Anleitungen zum Erstellen dieser Zertifikate und zum Anwenden dieser Zertifikate, um Kanäle für die Kommunikation zwischen dem Netzwerk Controller und den Verwaltungs Clients und der Southbound-Kommunikation mit Netzwerkgeräten, wie z. b. Software Load Balancer \(SLB\), zu sichern.
.
...
2. Die Authentifizierung zwischen Netzwerk Controller Knoten und in der Mitte befindlichen Geräten und Diensten, z. b. Hyper-V-Hosts und Software Load Balancer \(SLB-\).
...
- Der Wert für das Textfeld **restendpoint** muss entweder der voll qualifizierte Domänen Name des Netzwerk Controllers \(FQDN\) oder die IP-Adresse sein. 
- Der **restendpoint** -Wert muss mit dem Antragsteller Namen \(allgemeinen Namens, CN\) des X. 509-Zertifikats identisch sein.

### <a name="creating-a-self-signed-x509-certificate"></a>Erstellen eines selbst\-signierten X. 509-Zertifikats

Sie können ein selbst signiertes X. 509-Zertifikat erstellen und mit dem privaten Schlüssel exportieren, \(mit einem Kennwort geschützt\), indem Sie die folgenden Schritte für einzelne\-Knoten und mehrere\-Knoten Bereitstellungen von Netzwerk Controllern ausführen.

Wenn Sie selbst\-signierte Zertifikate erstellen, können Sie die folgenden Richtlinien verwenden.

- Sie können die IP-Adresse des Netzwerk Controller-Rest-Endpunkts für den DnsName-Parameter verwenden. Dies wird jedoch nicht empfohlen, da es erforderlich ist, dass sich die Netzwerk Controller Knoten in einem einzigen Verwaltungssubnetz befinden \(z. b. auf einem einzelnen Rack\)
- Bei bereit Stellungen mit mehreren Knoten NC wird der von Ihnen angegebene DNS-Name zum FQDN des Netzwerk Controller Clusters, \(DNS-Host A-Einträge automatisch erstellt werden.\) 
...
### <a name="creating-a-ca-signed-x509-certificate"></a>Erstellen eines ZS-\-signiertes X. 509-Zertifikat

Zum Erstellen eines Zertifikats mit einer Zertifizierungsstelle müssen Sie bereits eine Public Key-Infrastruktur \(PKI-\) mit Active Directory Zertifikat Dienste \(AD CS-\)bereitgestellt haben. 
...
>[!NOTE]
>Wenn der persönliche \(My – CERT: \ LocalMachine\MY\)-Zertifikat Speicher auf dem Hyper\-V-Host über mehr als ein X. 509-Zertifikat mit dem Antragsteller Namen (CN) als vollständig qualifizierter Domänen Namen des Hosts \(FQDN\)verfügt, stellen Sie sicher, dass das Zertifikat, das von Sdn verwendet wird, über eine zusätzliche benutzerdefinierte erweiterte Schlüssel Andernfalls funktioniert die Kommunikation zwischen dem Netzwerk Controller und dem Host möglicherweise nicht.
...
1. Klicken Sie auf dem **Server, auf**dem AD CS installiert ist, in Server-Manager auf Extras, und klicken Sie dann auf **Zertifizierungs**Stelle. Die Zertifizierungsstelle Microsoft Management Console \(MMC\) wird geöffnet. 
...
6.  Klicken Sie im Dialogfeld Vorlagen **Eigenschaften** auf der Registerkarte Antragsteller **Name** **in der Anforderung auf bereit**stellen. \(diese Einstellung ist für Netzwerk Controller-SSL-Zertifikate erforderlich.\)
...
2. Klicken Sie in der Konsolen Struktur auf **Zertifikate \(lokalen Computer\)** . Wählen Sie den **persönlichen** Zertifikat Speicher aus.
...
Nachdem Sie entweder ein selbst\-signiertes oder eine Zertifizierungsstelle\-signiertes Zertifikat erstellt haben, müssen Sie das Zertifikat mit dem privaten Schlüssel \(im PFX-Format exportieren\) und ohne den privaten Schlüssel \(im Zertifikat-Snap-in "Base-64. cer".\) 
...
2. Klicken Sie mit der rechten\-auf das Zertifikat, klicken Sie auf **Alle Tasks**und dann auf **exportieren**. Der Zertifikatexport-Assistent wird geöffnet. Klicken Sie auf **Weiter**.
...
7. Exportieren Sie das Zertifikat auch in. CER-Format. Hinweis: für den Export in. CER-Format, deaktivieren Sie die Option Ja, privaten Schlüssel exportieren.
...
Dies ist der Grund, warum für das SSL-Zertifikat die Clientauthentifizierungs-EKU konfiguriert sein muss. Dieses Zertifikat wird auf der Rest-Ressource "Servers" konfiguriert, \(Hyper-V-Hosts im Netzwerk Controller als Server Ressourcen\)dargestellt werden und angezeigt werden können, indem Sie den Windows PowerShell-Befehl **Get-networkcontrollerserver**ausführen.
...
Sie können das Zertifikat von einer Zertifizierungsstelle \(Zertifizierungsstelle\)registrieren. Wenn auf dem Host Computer kein ZS-basiertes Zertifikat gefunden wird, wird von SCVMM ein selbst signiertes Zertifikat erstellt und auf dem Host Computer bereitgestellt.
...
Beispiel für MUX-Rest-Ressourcen \(partielle\):
...
Netzwerk Controller und SLB MUX-Zertifikate müssen einander als vertrauenswürdig eingestuft werden, \(das Stamm Zertifikat des SLB MUX-Zertifikats im Speicher für vertrauenswürdige Stamm Zertifizierungsstellen des Netzwerk Controllers und umgekehrt\)vorhanden sein muss. Wenn Sie selbst\-signierte Zertifikate verwenden, stellt SCVMM sicher, dass die erforderlichen Zertifikate im Speicher Vertrauenswürdige Stamm Zertifizierungsstellen für den lokalen Computer vorhanden sind.
