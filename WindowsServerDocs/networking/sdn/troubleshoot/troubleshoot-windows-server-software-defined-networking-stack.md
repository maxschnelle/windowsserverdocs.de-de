...
* **Ungültige oder nicht unterstützte Konfiguration** Ein Benutzer ruft die Northbound-API falsch oder mit ungültiger Richtlinie auf.

* **Fehler in Richtlinien Anwendung** Die Richtlinie vom Netzwerk Controller wurde nicht an einen Hyper-v-Host übermittelt, die auf allen Hyper-v-Hosts (z. b. nach einer Livemigration) erheblich verzögert und/oder nicht auf dem neuesten Stand ist.
* **Konfigurations Abweichung oder Softwarefehler** Daten Pfad Probleme, die zu gelöschten Paketen führen.

* **Externer Fehler im Zusammenhang mit NIC-Hardware/-Treibern oder der zugrunde liegenden netzwerkfabric** Falsch konfigurierte Aufgaben Abladungen (z. b. VMQ) oder falsch konfigurierte netzwerkfabric (z. b. MTU)
...
Um die Diagnosetools für den Netzwerk Controller (Steuerungs Pfad) verwenden zu können, müssen Sie zunächst das Feature RSAT-networkcontroller installieren und das ``NetworkControllerDiagnostics`` Modul importieren:
...
Sie können auch die folgenden Parameter jedes Zertifikats überprüfen, um sicherzustellen, dass der Antragsteller Name den Erwartungen entspricht (Hostname oder NC-Rest-FQDN oder IP), dass das Zertifikat noch nicht abgelaufen ist und alle Zertifizierungsstellen in der Zertifikat Kette in der vertrauenswürdigen Stamm Zertifizierungsstelle enthalten sind.
...
Die SLB-Konfigurations Zustandsinformationen finden Sie im _diagnostics-slbstateResults.Js_ Datei in diesem Verzeichnis.
...
Abgesehen von den bisher angezeigten Problemen (insbesondere bei sdnexpress-basierten bereit Stellungen) ist der häufigste Grund für ein nicht konfiguriertes Mandanten Depot auf GW-VMS anscheinend die Tatsache, dass die GW-Kapazität in FabricConfig.psd1 kleiner ist als der Wert, der den Netzwerkverbindungen (S2S-Tunnel) in TenantConfig.psd1 zugewiesen wird. Dies lässt sich problemlos überprüfen, indem Ausgaben der folgenden Befehle verglichen werden:
...
1. Tenant Überprüfen Sie, ob für das virtuelle Subnetz oder die VM-Netzwerkschnittstellen keine verteilten Firewallrichtlinien angegeben sind, die den Datenverkehr blockieren würden.
...
1. Hoster Führen ``Get-ProviderAddress`` Sie auf beiden Hyper-v-Hosts, die die beiden fraglichen virtuellen Mandanten Computer ``Test-LogicalNetworkConnection`` ``ping -c <compartment>`` hosten, und dann oder vom Hyper-v-Host aus, um die Konnektivität im logischen HNV-anbieternetzwerk zu überprüfen.
...
3.  Hoster Wenn keine PA-IP-Adressen vorhanden sind und/oder die ca-Konnektivität beschädigt ist, überprüfen Sie, ob die Netzwerk Richtlinie empfangen wurde. Führen ``Get-PACAMapping`` Sie aus, um festzustellen, ob die Kapselungs Regeln und die ca-PA-Zuordnungen, die zum Erstellen von über Lagerungs Netzwerken erforderlich sind
4.  Hoster Überprüfen Sie, ob der Netzwerk Controller-Host-Agent mit dem Netzwerk Controller verbunden ist. Ausführen ``netstat -anp tcp |findstr 6640`` , um festzustellen, ob
...
- **Zentralisierter Protokoll Speicherort**.  Mit dem-Parameter können Sie den Speicherort ändern, an dem alle Protokolle gespeichert werden ``DiagnosticLogLocation`` .
- **Anmelde Informationen für den Zugriff auf den Protokoll Speicherort**  Mit dem-Parameter können Sie die Anmelde Informationen für den Zugriff auf den Protokoll Speicherort ändern ``LogLocationCredential`` .
- **Wechseln Sie zur lokalen Protokollierung**.  Wenn Sie einen zentralisierten Speicherort zum Speichern von Protokollen bereitgestellt haben, können Sie sich auf den Netzwerk Controller Knoten mit dem Parameter wieder lokal anmelden ``UseLocalLogLocation`` (nicht empfohlen aufgrund von umfangreichen Speicherplatzanforderungen).
...
2.  Überprüfen Sie die *sdnslbmperf-* Indikatoren in Perfmon auf einem der Netzwerk Controller Knoten-VMS (vorzugsweise der primäre Netzwerk Controller Knoten-Get-networkcontrollerreplica):
    ...
    4.  Ist SLBM mit muxes verbunden? (*Verbundene*  ==  muxes Fehlerhafte *muxes auf SLBM*  ==  *Muxes melden* fehlerfrei = # SLB muxes VMS).
...
**Remediation** Wiederherstellung Beheben Sie basierend auf den folgenden Diagnoseinformationen Folgendes:
