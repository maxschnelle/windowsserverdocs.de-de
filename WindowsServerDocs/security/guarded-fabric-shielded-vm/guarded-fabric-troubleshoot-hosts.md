---
title: Problembehandlung für geschützte Hosts
ms.topic: article
...
Erfolgreich                    | Der Host hat den Nachweis übermittelt.
TransientError            | Der letzte Nachweis Versuch ist aufgrund eines Netzwerk-, Dienst-oder anderen temporären Fehlers fehlgeschlagen. Wiederholen Sie den letzten Vorgang.
...
Unknown                   | Der Host hat noch nicht versucht, HGS zu bestätigen.
...
---------------------------|-------------------------------
BitLocker                  | Das Betriebssystem Volume des Hosts wird nicht von BitLocker verschlüsselt. Um dieses Problem zu beheben, [Aktivieren Sie BitLocker](/windows/security/information-protection/bitlocker/bitlocker-basic-deployment) auf dem Betriebssystem Volume, oder [Deaktivieren Sie die BitLocker-Richtlinie auf HGS](guarded-fabric-manage-hgs.md#review-attestation-policies).
...
Dumpencryption             | Der Host ist so konfiguriert, dass Absturz Abbilder oder Live-Speicher Abbilder zugelassen werden, aber diese Abbilder nicht verschlüsselt werden. Deaktivieren Sie entweder Abbilder auf dem Host, oder konfigurieren Sie die [dumpverschlüsselung](../../virtualization/hyper-v/manage/about-dump-encryption.md).
Dumpverschlüsselungskey          | Der Host ist so konfiguriert, dass er Dumps zulässt und verschlüsselt, verwendet aber kein Zertifikat, das HGS bekannt ist, um Sie zu verschlüsseln. Um dieses Problem zu beheben, [Aktualisieren Sie den Verschlüsselungsschlüssel](../../virtualization/hyper-v/manage/about-dump-encryption.md) für die Sicherung auf dem Host, oder [registrieren Sie den Schlüssel bei HGS](guarded-fabric-manage-hgs.md#authorizing-new-guarded-hosts).
FullBoot                   | Der Host wurde aus dem Ruhezustand oder dem Ruhezustand fortgesetzt. Starten Sie den Host neu, damit ein sauberer, vollständiger Start ermöglicht wird.
HibernationEnabled         | Der Host ist so konfiguriert, dass er den Ruhezustand zulässt, ohne die Ruhe Zustands Datei zu verschlüsseln, was von ihren HGS-Richtlinien nicht zulässig ist. Deaktivieren Sie den Ruhezustand, starten Sie den Host neu, oder konfigurieren Sie die [dumpverschlüsselung](../../virtualization/hyper-v/manage/about-dump-encryption.md).
...
Pagefileencryption         | Die Verschlüsselung der Auslagerungs Datei ist auf dem Host nicht aktiviert. Führen Sie aus, um die Verschlüsselung der Auslagerungs `fsutil behavior set encryptpagingfile 1` Datei zu aktivieren Weitere Informationen finden Sie unter "f"- [Verhalten](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/cc785435(v=ws.11)).
SecureBoot                 | Der sichere Start ist auf diesem Host nicht aktiviert oder verwendet nicht die Microsoft-Vorlage für den sicheren Start. [Aktivieren Sie den sicheren Start](/windows-hardware/manufacture/desktop/disabling-secure-boot#enable_secure_boot) mit der Microsoft Secure Boot-Vorlage, um dieses Problem zu beheben.
...
Weitere Informationen zu .NET 4,6 und TLS 1,0 finden Sie unter [Lösen des TLS 1,0-Problems, 2. Edition](/security/solving-tls1-problem).