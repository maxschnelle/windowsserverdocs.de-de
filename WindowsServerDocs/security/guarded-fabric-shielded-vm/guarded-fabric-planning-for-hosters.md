...
Stellen Sie den Host-Überwachungsdienst (Host Guardian Service, HGS) in einer hochgradig sicheren Umgebung bereit, die sich auf einem dedizierten physischen Server, einem abgeschirmten virtuellen Computer, einem virtuellen Computer auf einem isolierten Hyper-V-Host (getrennt von dem zu schützenden Fabric) oder einem logisch durch einen anderen Azure- Abgel.   
...
| Größenanpassung | Der HGS-Server Knoten der mittleren Größe (8 Kerne/4 GB) kann 1.000 Hyper-V-Hosts verarbeiten. |
| Management | Bestimmen Sie bestimmte Personen, die HGS verwalten werden. Sie sollten von Fabric-Administratoren getrennt sein. Zum Vergleich können sich HGS-Cluster auf die gleiche Weise wie eine Zertifizierungsstelle (Certificate Authority, ca) im Hinblick auf administrative Isolation, physische Bereitstellung und allgemeine Sicherheits Empfindlichkeit vorstellen. |
...
| Bereich | Details |
|------|---------|
| Hardware | <ul><li>Host Schlüssel Nachweis: Sie können beliebige vorhandene Hardware als überwachten Host verwenden. Es gibt einige Ausnahmen (um sicherzustellen, dass Ihr Host neue Sicherheitsmechanismen ab Windows Server 2016 verwenden kann, siehe [kompatible Hardware mit virtualisierungsbasiertem Windows Server 2016-Schutz der Code Integrität](guarded-fabric-compatible-hardware-with-virtualization-based-protection-of-code-integrity.md).</li><li>TPM-vertrauenswürdiger Nachweis: Sie können Hardware, die über die Hardware Assurance verfügt, über eine [zusätzliche Qualifizierung](https://msdn.microsoft.com/windows/hardware/commercialize/design/compatibility/systems#system-server-assurance) verfügen, sofern diese ordnungsgemäß konfiguriert ist (siehe [Server Konfigurationen, die mit abgeschirmten VMS kompatibel sind, und virtualisierungsbasierter Schutz der Code Integrität). ](guarded-fabric-compatible-hardware-with-virtualization-based-protection-of-code-integrity.md)für die spezifische Konfiguration). Dies umfasst TPM 2,0 und UEFI, Version 2.3.1 c und höher.</li></ul> |
