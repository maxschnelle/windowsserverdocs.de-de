...
ms.locfileid: "6121499"
---
# Bereitstellen von Nano Server
...
Sie können auch suchen und diese Pakete mit dem NanoServerPackage-Anbieter von "packagemanagement" (OneGet) PowerShell-Modul installieren. Beachten Sie hierzu den Abschnitt „Online-Installieren von Rollen und Features“ in diesem Thema.  
...
|PowerShell Desired State Configuration (DSC)|-Package Microsoft-NanoServer-DSC-Package<br />**Hinweis:** Ausführliche Informationen finden Sie unter [Verwenden von DSC unter Nano Server](https://msdn.microsoft.com/powershell/dsc/nanoDsc).|
|Internetinformationsdienste (IIS).|-Package Microsoft-NanoServer-IIS-Package<br />**Hinweis:** Weitere Informationen zum Arbeiten mit IIS finden Sie unter [IIS auf Nano Server](IIS-on-Nano-Server.md).|
|Hostunterstützung für Windows-Container|-Container|
|System Center Virtual Machine Manager-Agent|-Package Microsoft-NanoServer-SCVMM-Package<br />-Package Microsoft-NanoServer-SCVMM-Compute-Package<br />**Hinweis:** Verwenden Sie das SCVMM Compute-Paket nur, wenn Sie Hyper-V überwachen. Für zusammengeführte Bereitstellungen in VMM sollten Sie auch den Speicher-Parameter angeben. Weitere Informationen finden Sie in der [VMM Dokumentation](https://technet.microsoft.com/system-center-docs/vmm/manage/manage-compute-add-nano-hyper-v).| 
|System Center Operations Manager Agent| Wird separat installiert. Weitere Informationen zur System Center Operations Manager Dokumentation finden Sie https://technet.microsoft.com/system-center-docs/om/manage/install-agent-on-nano-server.|
...
### Von -Clustering-Parameter installierte Failovercluster-Elemente
...
### Vom -Speicherparameter installierte Datei- und Speicherelemente
...
### Installieren einer Nano Server-VHD  
...
### Installieren eines Nano Server-WIM  
...
### Lokales und remotes Bearbeiten von Dateien auf Nano Server  
...
### Installieren von Rollen und Features aus einem Paketrepository  
Sie können Nano Server-Pakete im Onlinepaketrepository suchen und daraus installieren, indem Sie den NanoServerPackage-Anbieter des PowerShell-Moduls „PackageManagement“ verwenden. Um diesen Anbieter zu installieren, verwenden Sie die folgenden Cmdlets:
...
### Suchen nach Nano Server-Paketen  
...
`Find-Package -ProviderName NanoServerPackage -Culture en-us` oder `Find-Package -Culture en-us -DisplayCulture` verwenden.
...
### Installieren von Nano Server-Paketen  
...
### Herunterladen von Nano Server-Paketen  

`Save-NanoServerPackage` bzw. `Save-Package` ermöglichen es Ihnen, Pakete herunterzuladen und zu speichern, ohne sie zu installieren. Beide Cmdlets nehmen Eingaben aus der Pipeline an.
...
### installierte Pakete inventarisieren
...
### Installieren von Rollen und Features von einer lokalen Quelle  
...
5.  Vergewissern Sie sich, dass das Paket und das zugehörige Language Pack ordnungsgemäß installiert wurde, indem Sie Folgendes ausführen:  
    ...
    **dism /online /get-packages**  
    ...
    Sie sollten den Text „Package Identity : Microsoft-NanoServer-IIS-Package~31bf3856ad364e35~amd64~en-US~10.0.10586.0“ zweimal sehen, einmal für den Versionstyp (Release Type) „Language Pack“ und einmal für den Versionstyp „Feature Pack“.  
  
## Anpassen einer vorhandenen Nano Server-VHD  
...
## Weitere mit „New-NanoServerImage“ und „Edit-NanoServerImage“ ausführbare Aufgaben  
  
### Beitreten zu Domänen  
...
### Herstellen einer Verbindung mit WinRM  
...
### Festlegen statischer IP-Adressen  
...
### Einbetten von benutzerdefinierten Daten  
...
### Ausführen von benutzerdefinierten Befehlen nach dem ersten Start
...
### Benutzerdefinierte PowerShell-Skripts als Teil der Erstellung von Abbildern ausführen
...
### Unterstützung für Entwicklungsszenarios
...
### Benutzerdefinierte Datei für die unbeaufsichtigte Installation  
...
## Installieren von Apps und Treibern
[comment]: # (Aus Xumin bug Sonne, #68620.)  
...
1. Installieren Sie [Windows 10 SDK](https://developer.microsoft.com/windows/downloads/windows-10-sdk) einschließlich der erforderlichen Tools zum Erstellen eines WSA-Pakets: MakeAppx, MakeCert, Pvk2Pfx, SignTool.
2. Deklarieren Sie ein Manifest: Beachten Sie das [WSA-Manifesterweiterungsschema](https://msdn.microsoft.com/library/windows/apps/mt670653.aspx), um die Manifestdatei „AppxManifest.xml“ zu erstellen.
...
#### Weitere Ressourcen für das Erstellen von Apps
...
- [So wird's gemacht: Erstellen eines allgemeinen Paketmanifests](https://msdn.microsoft.com/library/windows/desktop/br211475.aspx)
- [App Packager (MakeAppx.exe)](https://msdn.microsoft.com/library/windows/desktop/hh446767(v=vs.85).aspx)
- [How to create an app package signing certificate (So erstellen Sie ein App-Paketsignaturzertifikat)](https://msdn.microsoft.com/library/windows/desktop/jj835832(v=vs.85).aspx)
- [SignTool](https://msdn.microsoft.com/library/windows/desktop/aa387764(v=vs.85).aspx)

### Installieren von Treibern auf Nano Server
...
- [Driver Signing (Signieren von Treibern)](https://msdn.microsoft.com/windows/hardware/drivers/install/driver-signing)
- [Using a Universal INF File (Verwenden einer universellen INF-Datei)](https://msdn.microsoft.com/windows/hardware/drivers/install/using-a-configurable-inf-file)

#### Offline-Installieren von Treiberpaketen
...
#### Online-Installieren von Treiberpaketen
...
### So fügen Sie Nano Server online zu einer Domäne hinzu  
...
2.  Kopieren Sie die Datei „odjblob“ mit den folgenden Befehlen auf den Nano Server-Computer:  
    ...
    **net use z: \\\\\<IP-Adresse von Nano Server>\c$**  
...
7. Entfernen Sie aus Sicherheitsgründen den Nano Server mit dem folgenden Befehl aus der Liste der vertrauenswürdigen Hosts:
...
## Arbeiten mit Serverrollen auf Nano Server

###Verwenden von Hyper-V auf Nano Server  
...
Sie erstellen einen neuen Cluster mit `New-Cluster -Name <clustername> -Node <comma-separated cluster node list>`  
...
Um dem Cluster einen weiteren Knoten hinzuzufügen, verwenden Sie `Add-ClusterNode -Name <comma-separated cluster node list>  -Cluster <clustername>`  
  
Sie entfernen einen Knoten aus dem Cluster mit  `Remove-ClusterNode -Name <comma-separated cluster node list>  -Cluster <clustername>`  
  
Sie erstellen einen horizontal skalierbaren Dateiserver mit `Add-ClusterScaleoutFileServerRole -name <sofsname> -cluster <clustername>`  
...
### Verwenden von MPIO auf Nano Server
...
## Anhang: Beispieldatei „Unattend.xml“, mit der Nano Server zu einer Domäne hinzugefügt wird  
