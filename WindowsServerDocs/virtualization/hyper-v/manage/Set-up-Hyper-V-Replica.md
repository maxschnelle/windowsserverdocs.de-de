...
-   **Hyper-V-Hosts**: Primäre und sekundäre Host Server können sich physisch nebeneinander oder an separaten geografischen Standorten mit Replikation über eine WAN-Verbindung befinden. Hyper-V-Hosts können eigenständig, gruppiert oder eine Kombination aus beidem sein. Es gibt keine Active Directory Abhängigkeit zwischen den Servern, und Sie müssen keine Domänen Mitglieder sein.  

-   **Replikation und Änderungs Nachverfolgung**: Wenn Sie das Hyper-V-Replikat für eine bestimmte virtuelle Maschine aktivieren, wird durch die erste Replikation eine identische virtuelle Replikat Maschine auf einem sekundären Host Server Danach erstellt und verwaltet die Hyper-V-Replikat-Änderungs Nachverfolgung eine Protokolldatei, mit der Änderungen auf der VHD einer virtuellen Maschine erfasst werden. Die Protokolldatei wird auf der Grundlage der Replikations Häufigkeits Einstellungen in umgekehrter Reihenfolge an die Replikat-VHD Dies bedeutet, dass die neuesten Änderungen gespeichert und asynchron repliziert werden. Die Replikation kann über HTTP oder HTTPS erfolgen.  
...
-   **Failover**: Wenn ein Ausfall im primären (oder sekundären) Standort auftritt, können Sie ein Test-, geplantes oder ungeplantes Failover manuell initiieren.  

    ||Test|Geplant|Ungeplant|  
    |-|--------|-----------|-------------|  
    |Wann sollte ich ausgeführt werden?|Überprüfen, ob für einen virtuellen Computer ein Failover und Start am sekundären Standort möglich ist<br /><br />Nützlich für Tests und Schulungen|Während geplanter Ausfallzeiten und Ausfällen|Bei unerwarteten Ereignissen|  
...
-  So aktivieren Sie die Regeln auf einem eigenständigen Host Server:  
    ...
    2. Zum Aktivieren der HTTP-Authentifizierung (Kerberos) klicken Sie mit der rechten Maustaste auf **Hyper-V-Replikat-http-Listener (TCP-in)**  >-**Aktivierungs Regel** Klicken Sie zum Aktivieren der HTTPS-Zertifikat basierten Authentifizierung mit der rechten Maustaste auf **Hyper-V-Replikat-HTTPS-Listener (TCP-in)** > E-**able-Regel**.  
...
10. Wenn Sie die erweiterte (verkettete) Replikation konfigurieren möchten, öffnen Sie den Replikat Server, und klicken Sie mit der rechten Maustaste auf die zu replizierende virtuelle Maschine. Klicken Sie auf **Replikation** > **Replikation erweitern** und die Replikationseinstellungen angeben.  
