...
- **Failover**: Wenn ein Ausfall in Ihrem primären (oder sekundären) Standort auftritt, können Sie manuell ein Test-, geplantes oder ungeplantes Failover initiieren.
    ...
    |--|--|--|--|
    | Wann sollte ich diese ausführen? | Überprüfen, ob für einen virtuellen Computer ein Failover und Start am sekundären Standort möglich ist<p>Nützlich für Tests und Schulungen | Während geplanter Ausfallzeiten und Ausfällen | Bei unerwarteten Ereignissen |
...
-  So aktivieren Sie die Regeln auf einem eigenständigen Host Server:
    ...
    2. Zum Aktivieren der HTTP-Authentifizierung (Kerberos) klicken Sie mit der rechten Maustaste auf **Hyper-V-Replikat-http-Listener (TCP-in)**  >  **Regel aktivieren** Klicken Sie zum Aktivieren der Zertifikat basierten HTTPS-Authentifizierung mit der rechten Maustaste auf **Hyper-V-Replikat-HTTPS-Listener (TCP-in)**  >  **Regel aktivieren**.
