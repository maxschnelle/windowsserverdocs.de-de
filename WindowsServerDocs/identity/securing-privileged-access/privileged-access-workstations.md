...
|------|---------|
|Standardmäßig in PAW<p>– Windows Server Update Services<br />– Windows Defender|– Keine Zusatzkosten<br />– Führt erforderliche grundlegende Sicherheitsfunktionen aus<br />– Anweisungen in diesem Leitfaden enthalten|
|Verwalten mit [Intune](/mem/intune/)|<ul><li>Bietet cloudbasierte Transparenz und Kontrolle<p><ul><li>Softwarebereitstellung</li><li>Verwaltung von Softwareupdates</li><li>Verwaltung von Windows-Firewall-Richtlinien</li><li>Malwareschutz</li><li>Remoteunterstützung</li><li>Softwarelizenzverwaltung.</li></ul></li><li>Keine Serverinfrastruktur erforderlich</li><li>Erfordert das Ausführen der Schritte zum Aktivieren von Verbindungen mit Clouddiensten in Phase 2</li><li>Wenn der PAW-Computer keiner Domäne beigetreten ist, müssen mithilfe der im Download der Sicherheitsbaseline bereitgestellten Tools die SCM-Baselines auf die lokalen Images angewendet werden.</li></ul>|
...
|Tools für Sicherheitsüberprüfung und -überwachung, die Administratorzugriff erfordern|Hierzu gehören alle Tools, die einen Agent installieren oder ein Konto mit lokalem Administratorzugriff erfordern.<p>– Tools müssen die gleiche Sicherheitsebene aufweisen wie die PAWs<br />– Möglicherweise muss die Sicherheitsebene der PAWs heruntergestuft werden, um die Funktionsweise der Tools zu unterstützen (Öffnen von Ports, Installieren von Java oder anderer Middleware usw.), wodurch über einen Kompromiss hinsichtlich der Sicherheit entschieden werden muss|
|SIEM-Lösung (Security Information and Event Management)|<ul><li>Wenn die SIEM-Lösung keinen Agent erfordert:<p><ul><li>Zugriff auf Ereignisse auf PAWs ist durch Verwenden eines Kontos in der Gruppe **Ereignisprotokollleser** auch ohne Administratorrechte möglich</li><li>Erfordert das Öffnen von Netzwerkports zum Zulassen von eingehendem Datenverkehr von den SIEM-Servern</li></ul></li><li>Wenn SIEM einen Agent erfordert: **Security Scanning or monitoring tools requiring admin access**</li></ul>|
...
Die PAW-Lösung sollte gemäß den Standards unter [Prinzip der vertrauenswürdigen Quelle für betriebliche Standards](./securing-privileged-access-reference-material.md) betrieben werden.

## <a name="deploy-paws-using-a-guarded-fabric"></a>Bereitstellen von PAWs per geschütztem Fabric

Ein [geschütztes Fabric](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms-top-node.md) kann verwendet werden, um PAW-Workloads auf einem abgeschirmten virtuellen Computer über einen Laptop oder Sprungbrettserver auszuführen.
...
Eine ausführliche Beschreibung der Topologie und Sicherheitsversprechen von geschützten Fabrics findest du in der [Dokumentation zu geschützten Fabrics](../../security/guarded-fabric-shielded-vm/guarded-fabric-and-shielded-vms-top-node.md).
...
Nur die Computer, die dem Host-Überwachungsdienst bekannt sind und auf denen eine vertrauenswürdige [Codeintegritätsrichtlinie](/windows/security/threat-protection/windows-defender-application-control/windows-defender-application-control) ausgeführt wird, können abgeschirmte VMs starten.
...
[Schützen abgeleiteter Domänenanmeldeinformationen mit Credential Guard](/windows/security/identity-protection/credential-guard/credential-guard)
...
[Neuerungen in der Kerberos-Authentifizierung für Windows Server 2012](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/hh831747(v=ws.11))
