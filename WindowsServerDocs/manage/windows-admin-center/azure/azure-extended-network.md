...
2. Wählen Sie im Windows Admin Center die Option **Einstellungen** (in der oberen rechten Ecke der Seite) > **Erweiterungen** aus. Wählen Sie dann **Extensions** : ![ Screenshot mit der Registerkarte verfügbare Erweiterungen der Einstellungen aus.](../media/azure-extended-network/installed-extensions.png)
2. Wählen Sie auf der Registerkarte **Verfügbare Erweiterungen** die Option **Azure Extended Network** aus, und klicken Sie dann auf **Installieren**.
Nach wenigen Sekunden sollte eine Meldung angezeigt werden, die auf eine erfolgreiche Installation hinweist.
3. [Verbinden Sie das Windows Admin Center mit Azure](azure-integration.md), falls Sie dies noch nicht getan haben. Wenn Sie diesen Schritt jetzt überspringen, werden Sie später in diesem Prozess aufgefordert, dies zu tun.
4. Wechseln Sie im Windows Admin Center zu **alle Verbindungen**  >  **Hinzufügen** , und wählen Sie dann in der **Windows Server** -Kachel **Hinzufügen** aus. Geben Sie den Servernamen (und ggf. die Anmelde Informationen) für das lokale virtuelle Gerät ein.
![Screenshot des Windows Admin Centers, das das Azure Extended Network-Tool in Server-Manager auf dem lokalen virtuellen Gerät anzeigt ](../media/azure-extended-network/azure-extended-network.png) , klicken Sie auf **Azure Extended Network** , um zu beginnen. Wenn Sie zum ersten Mal eine Übersicht und eine Setup Schaltfläche angezeigt werden: ![ Bild](../media/azure-extended-network/azure-extended-network-intro.png)
...
> [!IMPORTANT] Scrollen Sie bei Bedarf nach unten, und klicken Sie auf **hochladen** , bevor Sie auf **Weiter klicken: Extended-Network Setup**.
...
Klicken Sie auf Weiter, nachdem Sie das Subnetz CIDR ausgewählt haben.
5. Wählen Sie das Abonnement, die Ressourcengruppe und das virtuelle Netzwerk aus, in das Sie sich erweitern: ![ Azure-Netzwerk: ](../media/azure-extended-network/azure-network.png) die Region (Azure-Standort) und das Subnetz werden automatisch ausgewählt. Wählen Sie weiter: Extended-Network Gateway-Setup aus, um fortzufahren.
6. Nun konfigurieren Sie die virtuellen Geräte. Die Informationen für das lokale Gateway sollten automatisch aufgefüllt ![ werden: Wenn das lokale Netzwerk Gateway ](../media/azure-extended-network/on-premises-network-gateway.png) korrekt aussieht, klicken Sie auf "weiter".
7. Für das virtuelle Azure-Gerät müssen Sie die Ressourcengruppe und den virtuellen Computer auswählen, die verwendet werden sollen: ![ Azure Network Gateway ](../media/azure-extended-network/azure-network-gateway.png) nach dem Auswählen des virtuellen Computers müssen Sie auch die Azure Extended-Network-gatewaysubnetz-CIDR auswählen. Klicken Sie dann auf Weiter: bereitstellen.
8. Überprüfen Sie die Zusammenfassungs Informationen, und klicken Sie dann auf bereitstellen, um mit der Die Bereitstellung dauert ungefähr 5-10 Minuten. Wenn die Bereitstellung fertiggestellt ist, wird der folgende Bereich zum Verwalten der erweiterten IP-Adressen angezeigt, und der Status sollte " **OK** : Installation fertig" lauten. ![](../media/azure-extended-network/installation-complete.png)
...
1. Klicken Sie auf **IPv4-Adressen hinzufügen** : ![ Image.](../media/azure-extended-network/add-ipv4-addresses.png)

2. Auf der rechten Seite wird das Flyout **neue IPv4-Adressen hinzufügen** angezeigt: ![ Bild](../media/azure-extended-network/add-ipv4-addresses-panel.png)
...
![Image](../media/azure-extended-network/add-ipv4-addresses-panel-filled.png)
