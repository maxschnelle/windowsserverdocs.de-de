...
| RDP-Einstellung                        | Beschreibung            | Werte                 | Standardwert          | Virtueller Windows-Desktop |
|------------------------------------|------------------------|------------------------|:----------------------:|:-----------------------:|
| full address:s:value | PC Name:</br>Diese Einstellung gibt den Namen oder die IP-Adresse des Remotecomputers an, mit dem eine Verbindung hergestellt werden soll.</br></br>Dies ist die einzige erforderliche Einstellung in einer RDP-Datei. | Ein gültiger Name, eine IPv4-Adresse oder eine IPv6-Adresse. | | |
| alternate full address:s:value | Gibt einen alternativen Namen oder eine alternative IP-Adresse des Remotecomputers an. | Ein gültiger Name, eine IPv4-Adresse oder eine IPv6-Adresse. | | |
...
| authentication level:i:value | Definiert die Einstellungen für die Ebene der Serverauthentifizierung. | - 0: Wenn bei der Serverauthentifizierung ein Fehler auftritt, ohne Warnung Verbindung mit dem Computer herstellen (Verbinden und mich nicht warnen)</br>- 1: Wenn bei der Serverauthentifizierung ein Fehler auftritt, keine Verbindung herstellen (Nicht verbinden)</br>- 2: Wenn bei der Serverauthentifizierung ein Fehler auftritt, Warnung anzeigen und mir ermöglichen, die Verbindung herzustellen oder abzulehnen (Mich warnen)</br>- 3: Kein Anforderungen an die Authentifizierung angegeben | 3 | |
| enablecredsspsupport:i:value | Legt fest, ob der Client den Credential Security Support Provider (CredSSP) für die Authentifizierung verwendet, sofern verfügbar. | - 0: RDP verwendet CredSSP nicht, selbst wenn dies vom Betriebssystem unterstützt wird</br>- 1: RDP verwendet CredSSP, wenn dies vom Betriebssystem unterstützt wird | 1 | X |
| disableconnectionsharing:i:value | Legt fest, ob der Client vorhandene getrennte Verbindungen mit Sitzungen wiederherstellt oder eine neue Verbindung initiiert, wenn eine neue Verbindung gestartet wird. | - 0: Erneut Verbindung mit einer vorhandenen Sitzung herstellen</br>- 1: Neue Verbindung initiieren | 0 | X |
| alternate shell:s:value | Gibt ein Programm an, das in der Remotesitzung anstelle von Explorer automatisch als Shell gestartet werden soll. | Gültiger Pfad zu einer ausführbaren Datei, z. B. „C:\Programme\Office\word.exe“ | | X |

## <a name="session-behavior"></a>Sitzungsverhalten

| RDP-Einstellung                        | Beschreibung            | Werte                 | Standardwert          | Virtueller Windows-Desktop |
|------------------------------------|------------------------|------------------------|:----------------------:|:-----------------------:|
| autoreconnection enabled:i:value | Legt fest, ob der Client automatisch versucht, erneut eine Verbindung mit dem Remotecomputer herzustellen, wenn die Verbindung getrennt wird (beispielsweise durch eine Unterbrechung der Netzwerkkonnektivität). | - 0: Der Client versucht nicht automatisch, eine Verbindung wiederherzustellen.</br>- 1: Der Client versucht automatisch, eine Verbindung wiederherzustellen. | 1 | X |
| bandwidthautodetect:i:value | Bestimmt, ob die automatische Erkennung des Netzwerktyps aktiviert ist. | - 0: Automatische Erkennung des Netzwerktyps deaktivieren</br>- 1: Automatische Erkennung des Netzwerktyps aktivieren | 1 | X |
| networkautodetect:i:value | Bestimmt, ob die automatische Erkennung der Netzwerkbandbreite verwendet werden soll. Erfordert, dass bandwidthautodetect auf „1“ festgelegt ist. | - 0: Automatische Erkennung der Netzwerkbandbreite nicht verwenden</br> - 1: Automatische Erkennung der Netzwerkbandbreite verwenden | 1 | X |
| compression:i:value | Bestimmt, ob die Massenkomprimierung bei der Übertragung an den lokalen Computer durch RDP aktiviert ist.|- 0: RDP-Massenkomprimierung deaktivieren</br>- 1: RDP-Massenkomprimierung aktivieren | 1 | X |
| videoplaybackmode:i:value| Legt fest, ob die Verbindung ein effizientes RDP-Multimediastreaming für die Videowiedergabe verwendet.|- 0: Kein effizientes RDP-Multimediastreaming für die Videowiedergabe verwenden</br>- 1: Effizientes RDP-Multimediastreaming für die Videowiedergabe verwenden | 1 | X |

## <a name="device-redirection"></a>Geräteumleitung

| RDP-Einstellung                        | Beschreibung            | Werte                 | Standardwert          | Virtueller Windows-Desktop |
|------------------------------------|------------------------|------------------------|:----------------------:|:-----------------------:|
| audiocapturemode:i:value | Mikrofonumleitung:</br>Gibt an, ob die Umleitung der Audioeingabe aktiviert ist. | - 0: Audioerfassung vom lokalen Gerät deaktivieren</br>- 1: Audioerfassung vom lokalen Gerät und Umleitung an eine Audioanwendung in der Remotesitzung aktivieren | 0 | X |
| encode redirected video capture:i:value | Aktiviert oder deaktiviert die Codierung umgeleiteter Videos. | - 0: Codierung umgeleiteter Videos deaktivieren</br>- 1: Codierung umgeleiteter Videos aktivieren | 1 | X |
| redirected video capture encoding quality:i:value | Steuert die Qualität codierter Videos. | - 0: Video mit hoher Komprimierung. Bei viel Bewegung kann dies die Qualität beeinträchtigen. </br>- 1: Mittlere Komprimierung</br>- 2: Video mit niedriger Komprimierung und hoher Bildqualität | 0 | X |
...
| RDP-Einstellung                        | Beschreibung            | Werte                 | Standardwert          | Virtueller Windows-Desktop |
|------------------------------------|------------------------|------------------------|:----------------------:|:-----------------------:|
| use multimon:i:value | Legt fest, ob die Remotesitzung eine oder mehrere Anzeigen auf dem lokalen Computer verwendet. | - 0: Unterstützung für mehrere Anzeigen nicht aktivieren</br>- 1: Unterstützung für mehrere Anzeigen aktivieren | 0 | X |
| selectedmonitors:s:value | Gibt an, welche lokalen Anzeigen von der Remotesitzung verwendet werden sollen. Die ausgewählten Anzeigen müssen zusammenhängend sein. Erfordert die Festlegung von multimon auf „1“.</br></br>Nur auf den Windows-Posteingangsclients (MSTSC) und Windows-Desktopclients (MSRDC) verfügbar. | Eine durch Trennzeichen getrennte Liste von computerspezifischen Anzeige-IDs. Die IDs können durch Aufrufen von „mstsc.exe /l“ abgerufen werden. Die erste aufgeführte ID wird als primäre Anzeige der Sitzung festgelegt. | Alle Anzeigen | X |
| maximizetocurrentdisplays:i:value | Legt fest, auf welcher Anzeige die Remotesitzung beim Maximieren in den Vollbildmodus wechselt. Erfordert die Festlegung von multimon auf „1“.</br></br>Nur auf dem Windows-Desktopclient (MSRDC) verfügbar. | - 0: Die Sitzung wird auf der Anzeige im Vollbildmodus angezeigt, die beim Maximieren anfänglich ausgewählt wurde.</br>- 1: Die Sitzung wechselt dynamisch auf den Anzeigen in den Vollbildmodus, auf denen das Sitzungsfenster beim Maximieren angezeigt wird. | 0 | X |
| singlemoninwindowedmode:i:value | Legt fest, ob eine Remotesitzung mit mehreren Anzeigen beim Beenden des Vollbildmodus automatisch zur einzelnen Anzeige wechselt. Erfordert die Festlegung von multimon auf „1“.</br></br>Nur auf dem Windows-Desktopclient (MSRDC) verfügbar. | - 0: Die Sitzung bleibt beim Beenden des Vollbildmodus auf allen Anzeigen erhalten.</br>- 1: Die Sitzung wechselt beim Beenden des Vollbildmodus zu einer einzelnen Anzeige. | 0 | X |
| screen mode id:i:value | Legt fest, ob das Remotesitzungsfenster beim Starten der Verbindung im Vollbildmodus angezeigt wird. | - 1: Die Remotesitzung wird in einem Fenster angezeigt</br>- 2: Die Remotesitzung wird im Vollbildmodus angezeigt | 2 | X |
| smart sizing:i:value | Legt fest, ob der lokale Computer den Inhalt der Remotesitzung auf Fenstergröße skaliert. | - 0: Der Inhalt des lokalen Fensters wird beim Ändern der Größe nicht skaliert.</br>- 1: Der Inhalt des lokalen Fensters wird beim Ändern der Größe skaliert. | 0 | X |
| dynamic resolution:i:value | Legt fest, ob die Auflösung der Remotesitzung automatisch aktualisiert wird, wenn die Größe des lokalen Fensters geändert wird. | - 0: Die Sitzungsauflösung bleibt für die Dauer der Sitzung statisch.</br>- 1: Die Sitzungsauflösung wird beim Ändern der Größe des lokalen Fensters aktualisiert. | 1 | X |
| desktop size id:i:value | Gibt die Maße des Remotesitzungsdesktops aus einer Reihe vordefinierter Optionen an. Diese Einstellung wird überschrieben, wenn „desktopheight“ und „desktopwidth“ angegeben sind.| -0: 640×480</br>- 1: 800×600</br>- 2: 1024×768</br>- 3: 1280×1024</br>- 4: 1600×1200 | 1 | X |
| desktopheight:i:value | Gibt die Auflösungshöhe der Remotesitzung (in Pixel) an. | Numerischer Wert zwischen 200 und 8.192 | Mit lokalem Computer abgleichen | X |
| desktopwidth:i:value | Gibt die Auflösungsbreite der Remotesitzung (in Pixel) an. | Numerischer Wert zwischen 200 und 8.192 | Mit lokalem Computer abgleichen | X |
| desktopscalefactor:i:value | Gibt den Skalierungsfaktor der Remotesitzung an, um Inhalte größer erscheinen zu lassen. | Numerischer Wert aus der folgenden Liste: 100, 125, 150, 175, 200, 250, 300, 400, 500 | 100 | X |
...
| remoteapplicationname:s:value | Gibt den Namen der RemoteApp in der Clientschnittstelle an, während die RemoteApp gestartet wird.| Anzeigename der App. Beispiel: Excel 2016 | | |
| remoteapplicationprogram:s:value | Gibt den Alias oder den Namen der ausführbaren Datei der RemoteApp an. | Gültiger Alias oder Name. Beispiel: EXCEL | | |
