...
**Fehler** (Client Ereignisprotokoll). Fehler bei der OTP-Zertifikat Registrierung auf dem Zertifizierungsstellen <username> Server <CA_name>, Anforderungs Fehler, mögliche Ursachen für Fehler: der ZS-Servername kann nicht aufgelöst werden. der Zugriff auf den Zertifizierungsstellen Server ist über den ersten DirectAccess-Tunnel nicht möglich, oder es kann keine Verbindung mit dem Zertifizierungsstellen Server hergestellt werden.
...
Wenn kein gültiges Zertifikat gefunden wird, löschen Sie das ungültige Zertifikat (sofern vorhanden), und melden Sie sich erneut für das Computer Zertifikat an, indem Sie entweder `gpupdate /Force` von einer Eingabeaufforderung mit erhöhten Rechten ausführen oder den Client Computer neu starten.
...
2.  Wenn keine Zertifizierungsstellen konfiguriert sind:
    ...
    2.  Wenden Sie die neue Konfiguration an, und erzwingen Sie, dass die Clients die DirectAccess-GPO-Einstellungen aktualisieren, indem Sie an `gpupdate /Force` einer Eingabeaufforderung mit erhöhten Rechten ausführen oder den Client Computer neu starten
...
Überprüfen Sie die konfigurierte DirectAccess-Server Adresse mithilfe von, `Get-DirectAccess` und korrigieren Sie die Adresse, wenn Sie falsch konfiguriert ist.

Stellen Sie sicher, dass die neuesten Einstellungen auf dem Client Computer bereitgestellt werden, indem Sie `gpupdate /force` von einer Eingabeaufforderung mit erhöhten Rechten ausführen oder den Client Computer neu starten.
...
**Fehler** (Client Ereignisprotokoll). Die Zertifikat Anforderung für die OTP-Authentifizierung kann nicht initialisiert werden. Ein privater Schlüssel kann nicht generiert werden, oder <username> der Benutzer kann nicht auf die Zertifikat Vorlage <OTP_template_name> auf dem Domänen Controller zugreifen.
...
**Fehler** (Client Ereignisprotokoll). Die Zertifizierungsstellen Vorlage, von der der Benutzer <username> ein Zertifikat angefordert hat, ist nicht zum Ausstellen von OTP-Zertifikaten konfiguriert.
...
1.  Überprüfen Sie den Namen der konfigurierten OTP-Signaturzertifikat Vorlage, indem Sie das PowerShell-Cmdlet ausführen `Get-DAOtpAuthentication` und den Wert von überprüfen `SigningCertificateTemplateName` .
...
Der für die <username> OTP-Authentifizierung angegebene Benutzername ist nicht vorhanden.
